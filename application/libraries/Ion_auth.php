...
class Ion_auth
{
	...
	public function __construct()
	{
		...
		$this->_cache_user_in_modules =& $this->ion_auth_model->_cache_user_in_modules;
	...
	public function is_modules($modules=NULL,$id = FALSE)
	{
		
		$this->ion_auth_model->trigger_events('is_modules');

		return $this->in_modules($modules, $id);
	}
	
	/**
	 * @param int|string|array $check_modules module(s) to check
	 * @param int|string|bool  $id          user id
	 * @param bool             $check_all   check if all module is present, or any of the module
	 *
	 * @return bool Whether the/all user(s) with the given ID(s) is/are in the given module
	 * @author Phil Sturgeon
	 **/
	public function in_modules($check_module, $id = FALSE, $check_all = FALSE)
	{
		$this->ion_auth_model->trigger_events('in_modules');

		$id || $id = $this->session->userdata('user_id');

		if (!is_array($check_module))
		{
			$check_module = array($check_module);
		}

		if (isset($this->_cache_user_in_modules[$id]))
		{
			$modules_array = $this->_cache_user_in_modules[$id];
		}
		else
		{
			$modules_groups = $this->ion_auth_model->get_modules($id)->result();
			
			$modules_array = array();
			foreach ($modules_groups as $group)
			{
				$modules_array[$group->id] = $group->module_name;
			}
			$this->_cache_user_in_modules[$id] = $modules_array;
		}
		
		foreach ($check_module as $key => $value)
		{
			
			$groups = (is_string($value)) ? $modules_array : array_keys($modules_array);
