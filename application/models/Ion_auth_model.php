...
class Ion_auth_model extends CI_Model
{
	...
	public $_cache_user_in_modules = array();
	 ...
	 * @author Ben Edmunds
	 */
	public function get_modules($id = FALSE)
	{
		
		$this->trigger_events('get_modules');

		// if no id was passed use the current users id
		$id || $id = $this->session->userdata('user_id');

		 return $this->db->select($this->tables['user_module_relation'].'.'.$this->join['modules'].' as id, '.$this->tables['modules'].'.module_name, '.$this->tables['modules'].'.status')
		                ->where($this->tables['user_module_relation'].'.'.$this->join['users'], $id)
		                ->join($this->tables['modules'], $this->tables['user_module_relation'].'.'.$this->join['modules'].'='.$this->tables['modules'].'.module_id')
		                ->get($this->tables['user_module_relation']);
